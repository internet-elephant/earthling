//
//  ContinentLookup.swift
//  Earthling
//
//  Created on 3/2/26.
//
//  Maps country names (as returned by MapKit) to continent names.
//  Used when saving an entry to organise files into the correct
//  continent folder. Returns "Unknown" for unrecognised countries,
//  which will create an Unknown/ folder rather than silently failing.
//
//  If MapKit returns a country name not in this list, add it here.
//  Country names must match MapKit's English localisation exactly.
//

import Foundation

struct ContinentLookup {

    /// Returns the continent name for a given country, or "Unknown" if not found.
